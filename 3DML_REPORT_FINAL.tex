...
\begin{titlepage}
  \centering
  \vspace*{1cm}
  \includegraphics[width=0.78\textwidth]{figures/logos/uca_logo_long.png}\par
  \vspace{1.4cm}
  {\Large Universite Clermont Auvergne\par}
  \vspace{0.2cm}
  {\large Master PAR - 3D Deep Learning\par}
  \vspace{1.2cm}
  {\huge\bfseries PointNet for 3D Point Cloud Classification\par}
  \vspace{0.4cm}
...
\newpage
\begin{abstract}
  In this report, we tested pointMLP, PointNetBasic, and PointNetFull on the ModelNet40\_PLY dataset.
  We also compared data augmentation methods to see which setup gives the best performance on ModelNet.
...
We follow the assignment structure and then add a separate section for additional experiments we ran (for example ensembling, normalization choices, and custom augmentation strategies).

\subsection{Dataset and task}
We use \textbf{ModelNet40\_PLY}, the standard benchmark also used in the original PointNet paper \cite{pointnet}, for 3D object classification from point clouds.
The dataset contains CAD models converted into point clouds with \textbf{1024 points} per object and \textbf{40 semantic classes}.
Typical classes include \textit{airplane}, \textit{chair}, \textit{sofa}, \textit{table}, \textit{car}, \textit{lamp}, \textit{monitor}, and \textit{guitar}.
In our local split used for all runs, we have \textbf{9843} training samples and \textbf{2468} test samples.
...
\addcontentsline{toc}{subsection}{Question 1: Test accuracy of PointMLP}
...
\addcontentsline{toc}{subsection}{Question 2: Comment (PointMLP)}
...
Figure~\ref{fig:arch_mlp} shows the main limitation of this model: flattening destroys the point-set structure. This explains why the model underfits geometry and confuses many classes in the confusion matrix.
From Figure~\ref{fig:cmp_mlp_basic}, PointMLP remains low on both train and test (about 29\% / 23\%), which indicates underfitting for this task.

\section{Exercise 2.1 -- Basic PointNet (without T-Nets)}
\subsection*{Question 1: Test accuracy of PointNetBasic}
\addcontentsline{toc}{subsection}{Question 1: Test accuracy of PointNetBasic}
...
\subsection*{Question 2: Comment}
\addcontentsline{toc}{subsection}{Question 2: Comment (PointNetBasic)}
...
\addcontentsline{toc}{subsection}{Question 1: Test accuracy with PointNet + input T-Net 3x3}
...
\addcontentsline{toc}{subsection}{Question 2: Comparison to PointNetBasic}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\linewidth]{pointnet_architecture_from_paper.jpg}
  \caption{Original PointNet architecture from the paper \cite{pointnet}. In our required implementation for this report, we use only the first input T-Net ($3\times3$) and do not use the second feature T-Net ($64\times64$).}
...
From the baseline run \texttt{r17}, the gap at the best-loss epoch is moderate (train acc 92.07\% vs test acc 86.59\%, about 5.48 points), which indicates that the input alignment helps generalization but does not eliminate all class ambiguity.

\section{Exercise 2.3 -- Data augmentation for 3D data}
\subsection*{Question 1: Proposed augmentation}
\addcontentsline{toc}{subsection}{Question 1: Proposed augmentation}
...
\addcontentsline{toc}{subsection}{Question 2: With vs without augmentation}
...
For this baseline recipe, RandomPointDropout mainly behaves like a robustness regularizer: NLL improves more reliably than top-1 accuracy. Also, some low-support classes (20 samples/class in the test split) move by large steps, so conclusions should be based on both global and class-level views.
...
Using an optimized training protocol (run \texttt{r16}):
\begin{itemize}[leftmargin=*]
  \item \textbf{Test accuracy}: \textbf{89.55\%}
  \item \textbf{Test NLL}: 0.4384
\end{itemize}
Compared with the strict baseline (86.59\%), this is a substantial gain and shows that recipe choices can matter as much as architecture depth for this task.
...
This is a substantial gain over the baseline (86.59\%) which shows that training choices can matter as much as architecture for this task.

\subsection{Optional full PointNet with feature T-Net $64\times64$}
We also evaluated the full 2 TNet variant and compared it against the 3*3 only architecture:
\begin{itemize}[leftmargin=*]
  \item (3x3 + 64x64, run \texttt{r14}): 89.51\%, NLL 0.4421
  \item (3x3 only, run \texttt{r16}): 89.55\%, NLL 0.4384
\end{itemize}
Difference is very small in this experiment. the 3x3 only model is slightly better here.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\linewidth]{compare_arch_full64_vs_required.png}
  \caption{Architecture comparison : 3x3-only vs full 3x3+64x64.}
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \caption{full architecture (input + feature T-Net) figures.}
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \caption{Additional figures for the full 64x64 variant.}
...
So we interpret the ensemble result as an upper bound of the current checkpoint family, not as a replacement for the required single-model benchmark.

\section{Other augmentation methods tested (not retained)}
Besides the required augmentation experiment, we also tested other custom augmentation methods:
\begin{itemize}[leftmargin=*]
  \item \textbf{RandomScaleShift} (custom geometric scaling + translation): isotropic scaling and random xyz shift to increase geometric variability.
  \item \textbf{RandomLocalPatchDropout} (custom local occlusion): remove a local neighborhood around a random pivot point, then refill to keep a fixed point count.
\end{itemize}

\noindent\textbf{Sources.}
RandomScaleShift follows the course/classical geometric augmentation logic and the official PointNet++ provider utilities \cite{pointnet2_provider}.
RandomLocalPatchDropout is inspired by local-region regularization from PointCutMix \cite{pointcutmix}.

\noindent\textbf{Observed outcome.}
In our additional runs, these stronger augmentation combinations reduced performance (test accuracy dropped to around \textbf{83\%}).
For this reason, we did not keep these methods in the final recipe and moved to other configurations.

\section{Final single-model adjustments (latest version)}
In this section, we explain additional adjustments and configurations we tested to improve and maximize PointNet accuracy as much as possible.
...
\noindent\textbf{Discussion.}
With this configuration, test accuracy stayed high across different epoch sweeps.
Even when changing the total training budget (45, 60, 70, 75, 90, 100), performance remained in a narrow high range around 89\%+, with the best value at 70 epochs.
This stability suggests that the result is not a single accidental run, but a robust behavior of the selected setup.
...
\midrule
Switch from PointMLP to PointNet shared MLP + max-pool & Overly aggressive or mismatched augmentation combinations \\
Input T-Net 3x3 alignment (vs Basic) & Rotation-vote test-time augmentation on aligned data \\
tuning (AdamW + cosine + clipping + smoothing) & Assuming full 64x64 feature T-Net is always better \\
A bit of geometric augmentation & Singlechange gains are small and sensitive to the chosen seed \\
Checkpoint ensembling for final accuracy boost & \\
Epoch sweep at fixed recipe (45--100) showed stable high performance around 89\%+ & Overly strong custom augmentation combinations (e.g., local patch dropout + stronger scale/shift) dropped accuracy to around 83\% \\
Single-model PointNetFull (3x3 T-Net only) + double dropout gave robust results & Increasing complexity alone does not guarantee improvement \\
\bottomrule
\end{tabular}
\end{table}
...
\section{Conclusion}
We addressed all questions in the statement with quantitative results and comparisons.
...
Our final single-model setup (PointNetFull with one T-Net, double dropout, mild augmentation) reached \textbf{89.91\%} test accuracy.
Across the epoch sweep (45 to 100), this configuration remained consistently high, with the best result at 70 epochs.
More aggressive custom augmentation combinations reduced performance (around 83\%) and were not retained.
As an optional upper bound, checkpoint ensembling reached 90\%+, but our main final result remains the single-model score above.
...
\bibitem{pointnet2_provider}
C. R. Qi et al., \textit{PointNet++ official provider (data augmentation utilities)},
\url{https://raw.githubusercontent.com/charlesq34/pointnet2/master/utils/provider.py}.

\bibitem{pointcutmix}
J. Zhang, L. Chen, B. Ouyang, B. Liu, J. Zhu, and Y. Chen,
\textit{PointCutMix: Regularization Strategy for Point Cloud Classification},
2021, \url{https://arxiv.org/abs/2101.01461}.
...
JMLR 2014, \url{https://jmlr.org/papers/v15/srivastava14a.html}.
\end{thebibliography}
