...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\linewidth]{training_curves_mlp.png}
  \caption{Exercise 1 (PointMLP): training and test loss/accuracy curves.}
...
The jump from 22.97\% to 85.17\% is mostly because we now have a better architectural assumption: the architecture now matches the structure of point sets (shared weights per point + symmetric global aggregation).

\section{Exercise 2.2 : PointNet with input T-Net $3\times3$}
We use the 3x3 only architecture with the first input T-Net.
...
The NLL also improves (0.5264 $\rightarrow$ 0.4834) which means we havebetter calibrated predictions.
...
\begin{figure}[H]
  ...
  \caption{(3x3 T-Net only): normalized test confusion matrix.}
\label{fig:r17_cm}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{PointMLP, PointNetBasic and PointNetFull comparison.}
...
\end{figure}
\noindent\textbf{Discussion.}
The T-Net increases the capacity and the invariance of the model. In Figures~\ref{fig:r17_curves} and \ref{fig:r17_cm}, we see that we have a better alignment of train/test behavior than Basic PointNet and a more diagonal confusion matrixfor several classes, consistent with improved canonical alignment of input clouds.
